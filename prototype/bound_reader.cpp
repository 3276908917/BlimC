...
Note: it is absolutely essential to be in a Python environment that can locate the pybind11 module.
Otherwise this line will fail (missing header file).
*/
...
{ 
	...
	curs += data_index + dtype_size * chan_start_idx;
	file.seekg(curs);
	...
	if (file.is_open()) {
		...
		for (int i = 0; i < n_int; i++)
			...
			for (int j = 0; j < n_if; j++) {
				offset = i * iScale + j * jScale; // This might overflow
...
PYBIND11_MODULE(bound_reader, m) {
	...
	m.def("read_sigproc_data", &read_sigproc_data, "File Reader (formally of type float32, but in practice should be castable to any desiderata)");
}
